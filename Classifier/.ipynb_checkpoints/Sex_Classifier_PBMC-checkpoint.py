...
def sex_classifier_pbmc(adata, class_prob_cutoff=0.85):
    ...
    if ('Sex' or 'sex') in adata.obs_keys():
        conditions = [((adata_test.obs.Sex_Class == adata_test.obs.Predictions) & (adata_test.obs.Class_Prediction > class_prob_cutoff)),
          ((adata_test.obs.Sex_Class != adata_test.obs.Predictions) & (adata_test.obs.Class_Prediction > class_prob_cutoff)), 
          ((adata_test.obs.Sex_Class == adata_test.obs.Predictions) & (adata_test.obs.Class_Prediction < class_prob_cutoff)),
          ((adata_test.obs.Sex_Class != adata_test.obs.Predictions) & (adata_test.obs.Class_Prediction < class_prob_cutoff))]
...
def misclassified(adata, min_ncounts=1100, min_genes=300, min_mtfrac=0.04, cutoff=0.85):
    ...
    if ('Sex' or 'sex') in adata.obs.keys():
        ...
        for cell in cells_annot:
            ...
            mispred = len(adata_cell[adata_cell.Sex != abc.Predictions])
        ...
        print('Misclassified will work on Class Prediction Cutoff')
